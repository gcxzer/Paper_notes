...
from tools.web_fetch.manifest import TOOL_GROUP
...
def register_web_fetch_tool(registry: ToolRegistry) -> None:
    ...
    if registry.get(WEB_FETCH_TOOL_NAME) is not None:
        return
    registry.register(ToolDefinition(
        name=WEB_FETCH_TOOL_NAME,
        description=(
            "Fetch and extract readable content from a specific public URL. "
            "Use this when the user provides a URL or when web_search snippets are insufficient. "
            "Supports public HTML, plain text, Markdown, JSON/XML-like text, and PDF URLs."
        ),
        parameters={
            "type": "object",
            "properties": {
