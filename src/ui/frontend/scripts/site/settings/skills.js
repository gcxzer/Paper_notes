...
function renderSkillDetail() {
  ...
  if (state.skillDetailLoading) {
    elements.skillsSettingsDetail.innerHTML = `<p class="settings-empty">Loading skill...</p>`;
    return;
  }
  if (!detail) {
    elements.skillsSettingsDetail.innerHTML = `<p class="settings-empty">Select a skill to view its instructions.</p>`;
    return;
  }
  if (!detail.success) {
...
function renderSkillsSettingsDialog() {
  ...
  if (state.skillsSettingsLoading) {
    elements.skillsSettingsList.innerHTML = `<p class="settings-empty">Loading skills...</p>`;
    renderSkillDetail();
    return;
  }
  if (!skills.length) {
    elements.skillsSettingsList.innerHTML = `<p class="settings-empty">${escapeHtml(settings.message || "No skills found.")}</p>`;
    state.selectedSkillDetail = null;
    renderSkillDetail();
    return;
  }
  if (!visibleSkills.length) {
    elements.skillsSettingsList.innerHTML = `<p class="settings-empty">No matching skills.</p>`;
    renderSkillDetail();
    return;
  }
