...
function positionProjectActionMenu(menu, row) {
  ...
  const buttonRect = row.querySelector(".ask-project-more")?.getBoundingClientRect() || rowRect;
...
function addSessionProjectOption(menu, { label, projectId, active, onClick }) {
  const button = document.createElement("button");
  button.className = `ask-session-menu-option ask-session-project-option ${active ? "is-active" : ""}`.trim();
  button.type = "button";
  button.dataset.projectId = normalizeText(projectId);
  button.innerHTML = `
