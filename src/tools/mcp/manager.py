...
def _tool_summary_from_definition(definition: MCPToolRecord) -> dict[str, Any]:
    metadata = definition.metadata or {}
    warnings = metadata.get("securityWarnings") if isinstance(metadata.get("securityWarnings"), list) else None
    return _mcp_tool_summary_payload(
...
def _mcp_tool_summary_payload(
    *,
    ...
    _attach_mcp_tool_metadata_summary(payload, annotations, has_output_schema)
    if warnings:
...
def _tool_summary(server: MCPServerTask, tool: Any, generated_name: str) -> dict[str, Any]:
    ...
    return _mcp_tool_summary_payload(
        ...
        has_output_schema=_mcp_tool_output_schema(tool, warnings=warnings) is not None,
        warnings=warnings,
    )
