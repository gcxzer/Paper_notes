...
PAPER_NOTES_WRITING_WORKFLOW_GUIDANCE = (
    ...
    "- For paper PDF content, use the available paper-reading tool. query_paper_content and read_paper are "
    "mutually exclusive: when RAG querying is enabled, only query_paper_content should be exposed; when RAG "
    "querying is disabled, only read_paper should be exposed. Do not try to call a paper-reading tool that is "
    "not in the available tools list. With query_paper_content, send one short retrieval query built from exact "
    "paper labels and keywords, not an expanded explanatory question. Treat numbered references as hard "
    "constraints: if the user asks about Figure 3, Table 2, Equation (4), Algorithm 1, Appendix C, or Section "
    "5.2, preserve that label in the query and add only paper keywords already supplied or known from context. "
    "In paper context, generic wording such as picture/image/visual plus a number usually means the numbered "
    "paper figure; query it as Figure N, not extracted image index N, unless the user explicitly asks for an "
    "extracted image file/index. For broad, comparative, or multi-part requests, synthesize those needs into "
    "one compact keyword query. If query_paper_content reports index_not_ready, tell the user to build the "
    "paper index or disable RAG querying to use direct PDF text reading. Use "
...
TOOL_GUIDANCE_BY_NAME = {
    ...
    "query_paper_content": (
        ...
        "figures/tables in context, limitations, conclusions, and section-level explanations. read_paper is the "
        "direct-PDF alternative in non-RAG mode and should not be available at the same time as "
        "query_paper_content. Use get_paper_context instead only when the user explicitly asks about library "
        "metadata, note HTML/sections, tags, annotations, or index status. "
        ...
        "If it reports index_not_ready, tell the user to build the paper index or switch off RAG querying for "
        "direct PDF text reading."
    ),
    "read_paper": (
        "Use read_paper to read local PDF text directly without RAG. Use action=search_text with a concise exact "
        "phrase, label, or keyword query for focused snippets. Use action=read_pages with page_start/page_end "
        "for raw page text or when RAG is disabled, unavailable, or index_not_ready. This tool reads extracted "
        "PDF text and may miss visual-only content; use inspect_paper_visuals for rendered pages or figures."
    ),
    "inspect_paper_visuals": (
        ...
        "paper text/content retrieval; use the exposed paper-reading tool for that."
    ),
    "manage_annotations": (
