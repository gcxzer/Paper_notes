...
from model_providers.providers.anthropic_provider import create_anthropic_chat_model
...
from model_providers.providers.google_provider import create_google_chat_model
...
def create_chat_model(config: AppConfig) -> Any:
    ...
    if provider == "anthropic":
        return create_anthropic_chat_model(model_config)
    if provider == "codex":
        return create_codex_chat_model(model_config)
    if provider == "google":
        return create_google_chat_model(model_config)
