...
class CodexAuthStore:
    ...
    The store intentionally never exposes tokens through status payloads. Real
    login/refresh will be wired here later.
    """

    def __init__(self, path: str | Path | None = None) -> None:
        self.path = Path(path) if path is not None else default_codex_auth_path()
    ...
    def status(self) -> CodexAuthStatus:
        ...
        return CodexAuthStatus(
            ...
            auth_store_path=_display_path(self.path),
        )
...
def _request_json(url: str, *, data: bytes, headers: dict[str, str]) -> tuple[int, dict[str, Any]]:
    request_headers = {**CODEX_CLIENT_HEADERS, **headers}
